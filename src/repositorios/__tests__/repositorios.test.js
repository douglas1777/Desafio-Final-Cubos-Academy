...
describe('manipular usuario', () => {
    ...
    test('consultaUsuario retorna o usuário correto', async () => {
        ...
        const [usuarioInserido] = await knexTest('usuarios').insert(usuario).returning(['id', 'nome', 'email']);
        ...
        const { senha: _, ...usuarioConsultado } = await consultaUsuario(usuarioInserido.id)

        expect(usuarioConsultado).toEqual({ id: usuarioInserido.id, nome: 'luci', email: 'luci@gmail.com' });
    });
    ...
    test('cadastraUsuario retorna um erro', async () => {
        ...
        try {
            await knexTest('usuarios').insert(usuarioErrado).returning(['id', 'nome', 'email'])
        } catch (error) {
    ...
    test('consultaUsuario retorna um erro', async () => {
        ...
        await knexTest('usuarios').insert(usuario).returning(['id', 'nome', 'email']);
    ...
    test('salva os dados do usuario no banco de dados', async () => {
        ...
        const salvarUsuario = async (usuario) => {
            return await knexTest("usuarios").insert(usuario).returning(['id', 'nome', 'email']);
        };

        const [usuarioCadastrado] = await salvarUsuario(usuario)

        expect(usuarioCadastrado).toEqual({ nome: 'luci', email: 'luci@gmail.com', id: usuarioCadastrado.id })
    ...
    test('atualiza os dados do usuario cadastrado no banco de dados', async () => {
        ...
        const [usuarioInserido] = await knexTest('usuarios').insert(usuario).returning('id');

        const atualizarUsuario = async (usuario, id) => {
            return await knexTest("usuarios").update(usuario).where({ id });
        };

        expect(await atualizarUsuario(usuarioAtualizado, usuarioInserido.id)).toBe(1)
    });
})
